...
if(SHOW_PROCESS)
    ...
    fig_poincare=figure('name','poincare');
...
%             h2.ZData = -1*ones(1,N);
   ...
   if(mod(k,1000)==0)
    figure(fig_poincare); hold on;
    col = [0.1 0.1 0.1 0.1];
    plot3(VLog(1,:),VLog(2,:),VLog(3,:),'.-','Color',col,'MarkerSize',2);
    VLog = [];
    drawnow;
   end
end
...
%% Plot Logged voltage in form of poincare plot
figure('name','poincare');
%plot(VLog(1,:),VLog(2,:),'.','Color',[1 0.1 0.1 0.01],'MarkerSize',1)
plot3(VLog(1,:),VLog(2,:),VLog(3,:),'o-','Color',[1 0.1 0.1 0.01],'MarkerSize',2)
