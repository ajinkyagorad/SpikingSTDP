...
[X,Xn,Tau,W,R,E]=createNetworkM([4 4 4],[2 1; -2 -1],2,[0.45 0.3; 0.2 0.15],0.2,2E-3);%(resSize,w,r0,k0,f_inhibit,tau,show)
%[X,Xn,Tau,W,R,E]=createNetworkD([4 4 4],[1 0 0],[2 1; -2 -1],2,[4 0.3; 0.2 0],0.2,2E-3);%(resSize,w,r0,k0,f_inhibit,tau,show)
%Tau = 1E-3*randi([1 20],length(X),1);i0
...
if(SHOW_PROCESS2)
    ...
    sim2_raster_plot = subplot(2,2,[1 2]);title('rasterPlot');
    sim2_param_plot = subplot(2,2,3); title('Weights');
    sim2_param2_plot = subplot(2,2,4); title('Reflection connection');
end
%figure('name','Input');
...
Fs = 1000;
dt = 1/Fs;
Tper = 0.4; % Time of each period for repeated input
Tsim = 1000*Tper; % T seconds
...
for k = 1:K %time in
    t = t+dt;
    k_ = mod(k,Tper*Fs);
    if(k_ == 0.01*Fs)%|| k_==0.015*Fs)%||k_==0.06*Fs||k_==0.20*Fs) % give impulse to only excitatory neurons
                Iapp = Iapp0*[zeros(1,floor(Nres/2)) ones(1,ceil(Nres/2)) zeros(1,Nout)];

    elseif(k_==0.21*Fs)
                Iapp = Iapp0*[ones(1,floor(Nres/2)) zeros(1,ceil(Nres/2)) zeros(1,Nout)];
    ...
    %% Display information
    if(k_==0 && SHOW_PROCESS2 == 1)
        
        figure(sim_fig2);
        subplot(sim2_raster_plot); 
        raster_plot(tSpikeLog,[],E);
        tSpikeLog = logical([]);
        subplot(sim2_param_plot);
        W_sparse=display_network_param(X,Xn,W,N);
        subplot(sim2_param2_plot);
        imagesc(W_sparse.*W_sparse');
        drawnow;
        if(SAVE_VIDEO)
        frame = getframe(gcf);
