...
[X,Xn,Tau,W,R,E]=createNetworkD([5 5 5],[1 1 1],[2 1; -2 -1],20,[1 1; 1 1],0.2,2E-3);
...
for k = 1:K %time in
    ...
    k_ = mod(k,Tper*Fs);
    Iapp = zeros(1,N);
    if(k_==0.06*Fs||k_==0.20*Fs) % give impulse to only excitatory neurons%k_>=0.01*Fs && k_<=0.2*Fs)
        Iapp(1) = Iapp0;
    end
    Iapp(E<0) = 0;
    spikeArrived(:) = 0;
