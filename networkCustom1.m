...
N_lin = 10;
X = [2:N_lin+1 ones(1,N_lin)]';
Xn = [ones(1,N_lin) 2:N_lin+1]';
Tau = 1E-3*[ones(1,N_lin) 100+[1:N_lin]]';
W = 5*ones(size(X));
R = [0 1:N_lin; 0 ones(1,N_lin);zeros(1,N_lin+1)]';
E = ones(1,N_lin+1)';
...
EL = 0; % V
Rp = 2E-3; %Sec
I0 = 2*3500E-7; % Synaptic current unit
...
for k = 1:K %time in
    ...
    k_ = mod(k,Tper*Fs);
    Iapp = zeros(1,N);
    if(k==0.01*Fs)%||k_==0.06*Fs||k_==0.20*Fs) % give impulse to only excitatory neurons
    Iapp(1) = 10*Iapp0;
    end
    Iapp(E<0) = 0;
    spikeArrived(:) = 0;
    ...
    if(~isempty(spikedNeurons))    % change weights for spiked neurons
        ...
        W(id_p_ex) = W(id_p_ex).*(1)-log(1+(W(id_p_ex).*0.01.*exp(-(t-tSaR(id_p_ex))/tauL)));
        W(id_d_ex) = W(id_d_ex).*(1)+log(1+(W(id_d_ex).*0.01.*exp(-(t+Tau(id_d_ex)-tSpike(X(id_d_ex))')/tauL)));
        W(id_p_in) = W(id_p_in).*(1)+log(1+(W(id_p_in).*0.01.*exp(-(t-tSaR(id_p_in))/tauL)));
        W(id_d_in) = W(id_d_in).*(1)-log(1+(W(id_d_in).*0.01.*exp(-(t+Tau(id_d_in)-tSpike(X(id_d_in))')/tauL)));
