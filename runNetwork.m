...
SHOW_PROCESS = 0;
tSpikeLogX= [];
for xdim = 1:10 % varpar loop
%% Give neural network input
[X,Xn,Tau,W,R,E]=createNetwork([xdim,5,5],[3 6; -2 -2],2,[0.45 0.3;0.6 0.3],0.2,1E-3);
Nres = length(E); % Neurons in the reservoir
% Not doing full network out neuron pool
display_network(X,Xn,Tau,W,R,E);
...
for k = 1:K %time in
    ...
    if(k == 0.05*Fs) % give impulse at 0.1s
        Iapp = Iapp0*ones(1,Nres);
    else
        Iapp = 0;
    end
    spikeArrived(:) = 0;
...
for i = 1:Nres
    tNSpike(i,:) = find(tSpikeLog(i,:),Nspikes);
end

figure('name','Sorted Rasterplots');
for Nspike = Nspikes % sort according to last common spike
    % order according to second time
    [~,orderN_id] = sort(tNSpike(:,Nspike));
    clf
    rasterNspike(tSpikeLog(orderN_id,:),tt,E);
    % possibly set color for each Nspike
    %plot(tSpikeLog(:,tNSpike(:,Nspike)),ones(1,Nres),'.','g');
    pause(0.01);
end
